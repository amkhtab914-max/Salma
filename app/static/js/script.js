 ...
 * This script is loaded on all pages via base.html.
 */

function updateDashboardUI(data) {
    // Find the elements. If they don't exist, this function does nothing.
    ...
    // Make sure we are on the dashboard page before trying to update
    if (!signalContent || !confidenceContent || !breakdownContent) {
        return;
    }
...
async function fetchRecommendation() {
    // Only try to fetch if we are on a page that has the recommendation elements
    if (document.getElementById('signal-content')) {
        try {
            ...
            updateDashboardUI({ status: 'Connection error' });
        }
    }
}
